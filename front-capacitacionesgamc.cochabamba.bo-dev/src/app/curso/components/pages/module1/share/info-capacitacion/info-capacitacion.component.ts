...
export class InfoCapacitacionComponent {
  ...
  toggleActivoCapacitacion() {
    this.capacitacionService.toggleActivoCapacitacion(this.traing.id)
      .subscribe(response => {
        if (response.ok) {
          ...
          Swal.fire({
            ...
            text: `La capacitación ha sido ${response.activo === 1 ? 'activada' : 'inactivada'} exitosamente.`,
            confirmButtonText: 'Aceptar',
          }).then(() => {
            window.location.reload();
          });
        } else {
          console.error('Error al cambiar el estado de la capacitación:', response.errors);
        }
      }, error => {
  ...
  calcularTotalSolicitadosIns() {
    //console.log('t',this.traing.capacitacion_inscripcion);
     return this.traing?.capacitacion_inscripcion
       .filter((inscripcion:any) => { return inscripcion.estado == 'PENDIENTE' } )
       .reduce((total:any) => total + 1, 0);
  }
  calcularTotalDisponiblesIns() {
    //console.log('t',this.traing.capacitacion_inscripcion);
     return this.traing?.cupo - this.traing?.capacitacion_inscripcion
       .filter((inscripcion:any) => { return inscripcion.estado == 'APROBADO' } )
       .reduce((total:any) => total + 1, 0) ;
  }
