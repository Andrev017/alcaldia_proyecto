...
export class CapacitacionService {
    ...
    getTrainingAllParameter(params: any): Observable<Capacitacion[]> {
        return this.http
            .get<CapactyRepons>(`${this.apiUrl}`, { params: params })
            .pipe(
                map((response) => {
    ...
    saveTraining(inputdata: any) {
        ...
        return this.http.post<CapactyRepons>(`${this.apiUrl}`, inputdata, {
            headers,
        });
    }
    updateTraining(inputdata: any, uuid: string) {
        const headers = new HttpHeaders(environment.httpHeaders);
        return this.http.put<CapactyRepons>(
            `${this.apiUrl + '/' + uuid}`,
            inputdata,
            { headers }
        );
    ...
    saveTrainingExternal(inputdata: any, fileUpload: any) {
        ...
        return this.http.post<CapactyRepons>(
            `${this.apiUrl + '/externa'}`,
            inputdata
        );
    }
