...
export class CursosCreateComponent implements OnInit {
  ...
  constructor(private service: CursoService, private router: Router, private fb: FormBuilder, private formErrorMessageService: FormErrorMessageService, public ref: DynamicDialogRef, public config: DynamicDialogConfig ) { 
      this.isAddMode = false;
    
      this.reactiveform = this.fb.group({
        //codigo: this.fb.control('', Validators.compose([Validators.required ]) ),
        nombre: this.fb.control('', Validators.compose([Validators.required, Validators.minLength(4) ]) ),
        contenido:  this.fb.control('', Validators.compose([Validators.required, Validators.minLength(10) ]) ),
        dirigido: this.fb.control('', Validators.compose([Validators.required, Validators.minLength(4) ])),
  ...
  updateCurso(){
    ...
    this.service.updateCurso(data, this.editdata.uuid )
    .subscribe({
      next:(resp) =>{
        this.service.saving.emit(true);
        this.closeDialog();  
