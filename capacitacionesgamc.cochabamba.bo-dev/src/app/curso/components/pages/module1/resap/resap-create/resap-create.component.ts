...
export class ResapCreateComponent {
  ...
  constructor( private serviceResap: ResapService, private router: Router, private fb: FormBuilder, public ref: DynamicDialogRef, public config: DynamicDialogConfig,private formErrorMessageService: FormErrorMessageService, private formatService: FormattingService, public dialogService: DialogService, private serviceTipoEva: TipoEvaService, private serviceCapacity: CapacitacionService, ) {
  ...
  listaTipoCriterio() {
    ...
    const params = { activo: "1", page:"", limit:"", nombretipo:"CUMPLIMIENTO DE OBJETIVO DE GESTIÓN" };
  ...
  verificarRegistro(){
    const params = { activo: "1", page:"", limit:"", id_inscripcion: this.traing.capacitacion_inscripcion[0].id  };
  ...
  createTraing(){
    ...
    this.serviceResap.saveResap(data)
    .subscribe({
      next:(resp) =>{
    ...
    for (let tipo of this.tipoCriterio ) {
      if( tipo.tipo_criterio )
        for (let criterio of tipo.tipo_criterio ) {
          ...
          resap.push({ id_inscripcion: this.traing.capacitacion_inscripcion[0].id , id_criterio_evaluacion: criterio.id, estado: valor});
        }      
    }
    const requestBody = {
      tipo:"coleccion",
