...
export class ResapService {
  ...
  saveResap(inputdata:any){
    ...
    return this.http.post< ResapResponse >( `${this.apiUrl}`, inputdata,{headers} );
  ...
  updateResap(inputdata:any, uuid:string){
    ...
    const headers = new HttpHeaders(environment.httpHeaders);
    return this.http.put<ResapResponse>( `${this.apiUrl+'/'+uuid}`, inputdata );
  }
  ...
  saveResapExternal(inputdata:any, fileUpload:any){
    ...
    return this.http.post< ResapResponse >( `${this.apiUrl+'/externa'}`, inputdata );
